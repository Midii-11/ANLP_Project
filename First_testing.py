# 2 ways of importing --> either specific transformer, or automatic --> Here I tested both methods to compare them.
# (automatic induces which transformer to use from the name used to initialize the model)
...
original_text = 'Junk foods taste good that’s why it is mostly liked by everyone of any age group especially kids and ' \
                ...
                'lethargy, inactiveness, and sleepiness. A person reflex becomes dull overtime and they lead an ' \
...
my_model1 = T5ForConditionalGeneration.from_pretrained('t5-small')
tokenizer1 = T5Tokenizer.from_pretrained('t5-small')

tokenizer2 = AutoTokenizer.from_pretrained('t5-base')
my_model2 = AutoModelForSeq2SeqLM.from_pretrained('t5-base', return_dict=True)

# Concatenating the word "summarize:" to raw text
# (The key-word "summarize" is used by T5/Bart/GPT-2 to recognise the text to summarize)
text = "summarize:" + original_text
print(text)

# encoding the input text
# (Each word from the original text receives a vector value (I think))
input_ids1 = tokenizer1.encode(text, return_tensors='pt', max_length=512, truncation='do_not_truncate')
input_ids2 = tokenizer2.encode(text, return_tensors='pt', max_length=512, truncation='do_not_truncate')
# print(input_ids)

# Generating summary ids
# Generate a new text, which is the summary of the first one
summary_ids1 = my_model1.generate(input_ids1, max_length=50)
summary_ids2 = my_model2.generate(input_ids2, max_length=50)
# print(summary_ids)

# Decoding the tensor and printing the summary.
t5_summary1 = tokenizer1.decode(summary_ids1[0])
t5_summary2 = tokenizer2.decode(summary_ids2[0])
print(t5_summary1)
print(t5_summary2)
